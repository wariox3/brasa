...
class ConsultasControlAccesosEmpleadosController extends Controller
{
    ...
    public function RegistradosAction() {
        $em = $this->getDoctrine()->getManager();
        $request = $this->getRequest();
        $paginator  = $this->get('knp_paginator');
        $form = $this->formularioListaRegistrados();
        $form->handleRequest($request);
        $this->listarRegistrados();
        if ($form->isValid()) {
            $arrSeleccionados = $request->request->get('ChkSeleccionar');
            if($form->get('BtnExcel')->isClicked()) {
                $this->filtrarListaRegistrados($form);
                $this->listarRegistrados();
                $this->generarRegistradosExcel();
            }            
        ...
        return $this->render('BrasaRecursoHumanoBundle:Consultas/ControlAcceso:empleadosregistrados.html.twig', array(
    ...
    private function listarRegistrados() {
        ...
        $em = $this->getDoctrine()->getManager();
        $this->strDqlLista = $em->getRepository('BrasaRecursoHumanoBundle:RhuHorarioAcceso')->listaRegistradosDql(                    
    ...
    private function formularioListaRegistrados() {
        $em = $this->getDoctrine()->getManager();
        $session = $this->getRequest()->getSession();
    ...
    private function filtrarListaRegistrados($form) {
        $session = $this->getRequest()->getSession();
        $request = $this->getRequest();
    ...
    private function generarRegistradosExcel() {
        $em = $this->getDoctrine()->getManager();
        $session = $this->getRequest()->getSession();
    ...
    public function NoRegistradosAction() {
        $em = $this->getDoctrine()->getManager();
        $request = $this->getRequest();
        $paginator  = $this->get('knp_paginator');
        $form = $this->formularioListaNoRegistrados();
        $form->handleRequest($request);
        $this->listarNoRegistrados();
        if ($form->isValid()) {
            $arrSeleccionados = $request->request->get('ChkSeleccionar');
            if($form->get('BtnExcel')->isClicked()) {
                $this->filtrarListaNoRegistrados($form);
                $this->listarNoRegistrados();
                $this->generarNoRegistradosExcel();
            }            
            if($form->get('BtnFiltrar')->isClicked()) {
                $this->filtrarListaNoRegistrados($form);
                $this->listarNoRegistrados();
        ...
        $arControlAccesosEmpleados = $paginator->paginate($em->createQuery($this->strDqlLista), $request->query->get('page', 1), 40);
        return $this->render('BrasaRecursoHumanoBundle:Consultas/ControlAcceso:empleadosnoregistrados.html.twig', array(
            'arControlAccesosEmpleados' => $arControlAccesosEmpleados,
            'form' => $form->createView()
            ));
    }        
    
    private function listarNoRegistrados() {
        $session = $this->getRequest()->getSession();
        $em = $this->getDoctrine()->getManager();
        $this->strDqlLista = $em->getRepository('BrasaRecursoHumanoBundle:RhuHorarioAcceso')->listaNoRegistradosDql(                    
            $session->get('filtroIdentificacion'),    
            $session->get('filtroNombre'),
            $session->get('filtroDesde'),
            $session->get('filtroHasta')
    ...
    private function formularioListaNoRegistrados() {
        $em = $this->getDoctrine()->getManager();
        $session = $this->getRequest()->getSession();
    ...
    private function filtrarListaNoRegistrados($form) {
        $session = $this->getRequest()->getSession();
        $request = $this->getRequest();
        $controles = $request->request->get('form');
        $session->set('filtroIdentificacion', $form->get('TxtNumeroIdentificacion')->getData());
    ...
    private function generarNoRegistradosExcel() {
        $em = $this->getDoctrine()->getManager();
        $session = $this->getRequest()->getSession();
        $objPHPExcel = new \PHPExcel();
        // Set document properties

...
class FormatoProcesoDisciplinarioEstelar extends \FPDF_FPDF {
    ...
    public function Generar($miThis, $codigoProcesoDisciplinarioTipo,$codigoProcesoDisciplinario) {        
        ...
        $pdf = new FormatoProcesoDisciplinarioEstelar();
        $pdf->AliasNbPages();
        $pdf->AddPage();
    ...
    public function Header() {
        ...
        if ($arContenidoFormatoA->getRequiereFormatoIso() == 1){
            ...
            $this->SetXY(60, 10);
            $this->Cell(140, 10, utf8_decode("SEGURIDAD ESTELAR LTDA"), 0, 0, 'C', 1); //cuadro mitad arriba
            $this->SetXY(60, 20);            
            $this->Cell(140, 10, utf8_decode("SISTEMA DE GESTIÓN INTEGRAL"), 0, 0, 'C', 1); //cuardo mitad medio
            $this->SetFont('Arial','B',10);
            $this->SetXY(60, 30);
            $this->Cell(40, 10, utf8_decode("CODIGO: FO.GH .023.00"), 1, 0, 'C', 1); //cuardo mitad abajo
            $this->Cell(40, 10, utf8_decode('PAGINA ') . $this->PageNo() . ' de {nb}', 1, 0, 'C', 1); //cuardo mitad abajo
            $this->Cell(40, 10, utf8_decode("FECHA: 12/11/2013"), 1, 0, 'C', 1); //cuardo mitad abajo
            $this->SetXY(60, 40);
            $this->SetFont('Arial','B',8);
            $this->Cell(140, 10, utf8_decode("FORMATO PARA COMUNICACIÓN INTERNA Y EXTERNA"), 1, 0, 'C', 1); //cuardo mitad abajo
            //$this->SetXY(150, 10);

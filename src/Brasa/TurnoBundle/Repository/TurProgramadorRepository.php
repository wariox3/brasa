...
class TurProgramadorRepository extends EntityRepository {
    
    public function listaDql($codigoProgramador = '', $strNombre = "", $codigoCliente = '') {
        $em = $this->getEntityManager();
        $dql   = "SELECT p FROM BrasaTurnoBundle:TurProgramador p WHERE p.codigoProgramadorPk <> 0 ";
        if($codigoCliente != "" ) {
        ...
        if($codigoProgramador != "" ) {
            $dql .= " AND p.codigoProgramadorPk = " . $codigoProgramador;
        }
    ...
    public function eliminar($arrSeleccionados) {
        ...
        if(count($arrSeleccionados) > 0) {
            foreach ($arrSeleccionados AS $codigo) {
                $ar = $em->getRepository('BrasaTurnoBundle:TurProgramador')->find($codigo);
                $em->remove($ar);
            }
            $em->flush();
        }

...
class InvMovimientosRepository extends EntityRepository {
    ...
    public function Autorizar($codigoMovimiento) {
        try {
            ...
            if ($arMovimiento->getEstadoAutorizado() == 0) {
                ...
                $arMovimientosDetalle = new \Brasa\InventarioBundle\Entity\InvMovimientosDetalles();
                $arMovimientosDetalle = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->findBy(array('codigoMovimientoFk' => $codigoMovimiento));
                if(count($arMovimientosDetalle) > 0) {
                    ...
                    if ($arDocumento->getOperacionInventario() != 0 || $arMovimiento->getDocumentoRel()->getCodigoDocumentoTipoFk() == 9) {
                        ...
                        if (($arDocumento->getOperacionInventario() == -1 || $arMovimiento->getDocumentoRel()->getCodigoDocumentoTipoFk() == 9) && $strValidacion == "") {
                            $intPosicion = 0;
                            $arMovDetLoteItem = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->DevMovDetItemLote($codigoMovimiento);
                            while ($strValidacion == "" && $intPosicion < count($arMovDetLoteItem)) {
                                ...
                                if($arMovDetLoteItem[$intPosicion]['itemServicio'] == 0) {
                                    if(!$em->getRepository('BrasaInventarioBundle:InvLotes')->ValidarDisponible($arMovDetLoteItem[$intPosicion]['codigoItemFk'], $arMovDetLoteItem[$intPosicion]['codigoBodegaFk'], $arMovDetLoteItem[$intPosicion]['loteFk'], $arMovDetLoteItem[$intPosicion]['cantidad']))
                    ...
                    if ($arMovimiento->getDocumentoRel()->getCodigoDocumentoTipoFk() == 10) {
                        ...
                        while ($strValidacion == "" && $intPosicion < count($arMovimientosDetalle)) {
                            ...
                            if ($arMovimientosDetalle[$intPosicion]->getCodigoBodegaDestinoFk() != "") {
                                ...
                                $arBodega = $em->getRepository('BrasaInventarioBundle:InvBodegas')->find($arMovimientosDetalle[$intPosicion]->getCodigoBodegaDestinoFk());
                                if(count($arBodega) <= 0)
                                    $strValidacion .= "No ha especificado una a bodega destino valida.";
                            }
                            if($strValidacion != "")
                        ...
                        if ($strValidacion == "") {
                            ...
                            while ($strValidacion == "" && $intPosicion < count($arMovDetLoteItem)) {
                                //No aplica para items de servicio
                                if($arMovDetLoteItem[$intPosicion]['itemServicio'] == 0) {
                                    if(!$em->getRepository('BrasaInventarioBundle:InvLotes')->ValidarExistencia($arMovDetLoteItem[$intPosicion]['codigoItemFk'], $arMovDetLoteItem[$intPosicion]['codigoBodegaFk'], $arMovDetLoteItem[$intPosicion]['loteFk'], $arMovDetLoteItem[$intPosicion]['cantidad']))
                                        $strValidacion = "Error validando las existencias del Item: " . $arMovDetLoteItem[$intPosicion]['codigoItemFk'] . " Lote " . $arMovDetLoteItem[$intPosicion]['loteFk'] . " Bodega " . $arMovDetLoteItem[$intPosicion]['codigoBodegaFk'];
                                }
                                $intPosicion++;
                            }
                    ...
                    if ($strValidacion == "") {
                        foreach ($arMovimientosDetalle as $arMovimientoDetalle) {
                            ...
                            if ($intOperacionInventario != 0) {
                                //Mueve la existencia
                                $em->getRepository('BrasaInventarioBundle:InvLotes')->MoverExistencia($arMovimientoDetalle->getCodigoItemFk(), $arMovimientoDetalle->getCodigoBodegaFk(), $arMovimientoDetalle->getLoteFk(), $arDocumento->getOperacionInventario(), $arMovimientoDetalle->getCantidad(), $arMovimientoDetalle->getFechaVencimiento());
                                //Mueve remisiones
                                if($arMovimientoDetalle->getAfectarRemision() == 1) {
                                    $em->getRepository('BrasaInventarioBundle:InvLotes')->MoverRemision($arMovimientoDetalle->getCodigoItemFk(), $arMovimientoDetalle->getCodigoBodegaFk(), $arMovimientoDetalle->getLoteFk(), -1, $arMovimientoDetalle->getCantidad(), $arMovimientoDetalle->getFechaVencimiento());                                
                                }
                            }
                            ...
                            //Traslados
                            if($arMovimiento->getDocumentoRel()->getCodigoDocumentoTipoFk() == 10) {
                                $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->CrearDetalleTraslado($arMovimientoDetalle->getCodigoDetalleMovimientoPk());
                            }
                        }

                        //Ejecutar inventario traslados
                        if($arMovimiento->getDocumentoRel()->getCodigoDocumentoTipoFk() == 10) {
                            $arMovimientosDetalleTraslado = new \Brasa\InventarioBundle\Entity\InvMovimientosDetalles();
                            $arMovimientosDetalleTraslado = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->findBy(array('codigoMovimientoFk' => $codigoMovimiento));
                        ...
                        if($arMovimiento->getNumeroMovimiento() == 0) {
                            if($arDocumento->getAsignarConsecutivoImpresion() == 0)
                                $arMovimiento->setNumeroMovimiento ($em->getRepository('BrasaInventarioBundle:InvMovimientos')->DevConsecutivo($arDocumento->getCodigoDocumentoPk()));
                        }
                ...
                if($arMovimiento->getValorDescuentoFinanciero() <= 0) {
                    ...
                    if ($strValidacion == "") {
                        ...
                        foreach ($arMovimientosDetalle as $arMovimientoDetalle) {
                            ...
                            if ($arMovimiento->getDocumentoRel()->getOperacionInventario() != 0) {
                                $em->getRepository('BrasaInventarioBundle:InvLotes')->MoverExistencia($arMovimientoDetalle->getCodigoItemFk(), $arMovimientoDetalle->getCodigoBodegaFk(), $arMovimientoDetalle->getLoteFk(), $arMovimientoDetalle->getOperacionInventario() * -1, $arMovimientoDetalle->getCantidad());
                                //Mueve remisiones
                                if($arMovimientoDetalle->getAfectarRemision() == 1) {
                                    $em->getRepository('BrasaInventarioBundle:InvLotes')->MoverRemision($arMovimientoDetalle->getCodigoItemFk(), $arMovimientoDetalle->getCodigoBodegaFk(), $arMovimientoDetalle->getLoteFk(), 1, $arMovimientoDetalle->getCantidad(), $arMovimientoDetalle->getFechaVencimiento());                                
                                }                                
                        ...
                        if($arMovimiento->getDocumentoRel()->getCodigoDocumentoTipoFk() == 10) {
                            ...
                            foreach ($arMovimientosDetalleTraslado as $arMovimientoDetalleTraslado) {
                                if ($arMovimientoDetalleTraslado->getOperacionInventario() != 0) {
                                    ...
                                    $arMovimientoDetalleEliminar = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->find($arMovimientoDetalleTraslado->getCodigoDetalleMovimientoPk());
                    ...
                    if($arMovimiento->getDocumentoRel()->getCodigoDocumentoTipoFk() == 10) {
                        ...
                        foreach ($arMovimientosDetalleTraslado as $arMovimientoDetalleTraslado) {
                            if ($arMovimientoDetalleTraslado->getOperacionInventario() != 0) {
                                ...
                                $arMovimientoDetalleAct = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->find($arMovimientoDetalleTraslado->getCodigoDetalleMovimientoPk());
                                $arMovimientoDetalleAct->setEstadoAnulado(1);
                                $arMovimientoDetalleAct->setCantidad(0);
                                $arMovimientoDetalleAct->setPrecio(0);
                                $arMovimientoDetalleAct->setPorcentajeDescuento(0);
        ...
        if($arMovimiento->getEstadoImpreso() == 0) {
            if ($arMovimiento->getEstadoAutorizado() == 1 && $arMovimiento->getEstadoAnulado() == 0) {
                if($arMovimiento->getNumeroMovimiento() == 0 && $arMovimiento->getDocumentoRel()->getAsignarConsecutivoImpresion() == 1 && $arMovimiento->getEstadoImpreso() == 0) {
                    $arMovimiento->setNumeroMovimiento ($em->getRepository('BrasaInventarioBundle:InvMovimientos')->DevConsecutivo($arMovimiento->getCodigoDocumentoFk()));
                    $arMovimiento->setFecha(date_create(date('Y-m-d H:i:s')));
                }
                ...
                if ($arMovimiento->getDocumentoRel()->getGeneraCartera() == 1)
                    $em->getRepository('BrasaCarteraBundle:CarCuentasCobrar')->Aplicar($codigoMovimiento);
                if ($arMovimiento->getDocumentoRel()->getGeneraTesoreria() == 1)
                    $em->getRepository('BrasaTesoreriaBundle:TesCuentasPagar')->Aplicar($codigoMovimiento);
    ...
    public function Liquidar($codigoMovimiento) {
        ...
        $arConfiguraciones = new \Brasa\GeneralBundle\Entity\GenConfiguraciones();
        $arConfiguraciones = $em->getRepository('BrasaGeneralBundle:GenConfiguraciones')->find(1);
        $arMovimiento = new \Brasa\InventarioBundle\Entity\InvMovimientos();
        $arMovimiento = $em->getRepository('BrasaInventarioBundle:InvMovimientos')->find($codigoMovimiento);
        $arMovimientosDetalle = new \Brasa\InventarioBundle\Entity\InvMovimientosDetalles();
        $arMovimientosDetalle = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->findBy(array('codigoMovimientoFk' => $codigoMovimiento));
        ...
        $arMovimientoAct = new \Brasa\InventarioBundle\Entity\InvMovimientos();
        $arMovimientoAct = $em->getRepository('BrasaInventarioBundle:InvMovimientos')->find($codigoMovimiento);
    ...
    public function DevMovimientosFiltro($codigoDocumento, $codigoMovimiento, $numeroMovimiento, $codigoTercero, $ChkFecha, $strFechaDesde, $strFechaHasta, $boolAutorizado = 0, $boolImpreso = 0) {
        ...
        $query = $em->createQueryBuilder()
                ->select('m')
                ->from('BrasaInventarioBundle:InvMovimientos', 'm')
        ...
        $arMovimientos = new \Brasa\InventarioBundle\Entity\InvMovimientosDetalles();
    ...
    public function DevEstadoMovimiento($codigoMovimientoPk) {
        $em = $this->getEntityManager();
        $arMovimiento = new \Brasa\InventarioBundle\Entity\InvMovimientos();
        $arMovimiento = $em->getRepository('BrasaInventarioBundle:InvMovimientos')->find($codigoMovimientoPk);
    ...
    public function DevConsecutivo ($intCodigoDocumento) {
        $em = $this->getEntityManager();
        $arDocumento = new \Brasa\InventarioBundle\Entity\InvDocumentos();
        $arDocumento = $em->getRepository('BrasaInventarioBundle:InvDocumentos')->find($intCodigoDocumento);

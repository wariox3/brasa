...
/**
 * CtbRegistrosRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class CtbRegistrosRepository extends EntityRepository
{
        ...
        if($arMovimiento->getEstadoContabilizado() == 0) {
            ...
            if($arMovimiento->getDocumentoRel()->getCodigoDocumentoTipoFk() == 1 && $strResultado == "") {
                ...
                if($arMovimiento->getDocumentoRel()->getCodigoComprobanteContableFk() == 4)
                    $arMovimientosDetalles = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->DevResumenCuentaCompras($codigoMovimiento);
                //Para las devoluciones de compra
                if($arMovimiento->getDocumentoRel()->getCodigoComprobanteContableFk() == 11)
                    $arMovimientosDetalles = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->DevResumenCuentaDevolucionCompras($codigoMovimiento);
                ...
                foreach($arMovimientosDetalles as $arMovimientoDetalle) {
                    ...
                    //Para las compras
                    if($arMovimiento->getDocumentoRel()->getCodigoComprobanteContableFk() == 4)
                        $arCuentaContable = $em->getRepository('BrasaContabilidadBundle:CtbCuentasContables')->find($arMovimientoDetalle['cuentaCompras']);

                    //Para las devoluciones de compra
                    if($arMovimiento->getDocumentoRel()->getCodigoComprobanteContableFk() == 11)
                        $arCuentaContable = $em->getRepository('BrasaContabilidadBundle:CtbCuentasContables')->find($arMovimientoDetalle['cuentaDevolucionCompras']);
            ...
            if($arMovimiento->getDocumentoRel()->getCodigoDocumentoTipoFk() == 4) {
                ...
                $arMovimientosDetalles = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->DevResumenCuentaCostoVentas($codigoMovimiento);
                foreach($arMovimientosDetalles as $arMovimientoDetalle) {
                    $arRegistro = $this->CrearEncabezadoRegistro($arMovimiento);
                    $arCuentaContable = $em->getRepository('BrasaContabilidadBundle:CtbCuentasContables')->find($arMovimientoDetalle['cuentaCosto']);
                    $arRegistro->setCuentaRel($arCuentaContable);                                       
                ...
                if($arMovimiento->getDocumentoRel()->getCodigoComprobanteContableFk() == 3)
                    $arMovimientosDetalles = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->DevResumenCuentaVentas($codigoMovimiento);
                //Para las devoluciones de ventas
                if($arMovimiento->getDocumentoRel()->getCodigoComprobanteContableFk() == 13)
                    $arMovimientosDetalles = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->DevResumenCuentaDevolucionVentas($codigoMovimiento);

                foreach($arMovimientosDetalles as $arMovimientoDetalle) {
                    $arRegistro = $this->CrearEncabezadoRegistro($arMovimiento);
                    //Para las ventas
                    if($arMovimiento->getDocumentoRel()->getCodigoComprobanteContableFk() == 3)
                        $arCuentaContable = $em->getRepository('BrasaContabilidadBundle:CtbCuentasContables')->find($arMovimientoDetalle['cuentaVentas']);

                    //Para las devoluciones de venta
                    if($arMovimiento->getDocumentoRel()->getCodigoComprobanteContableFk() == 13)
                        $arCuentaContable = $em->getRepository('BrasaContabilidadBundle:CtbCuentasContables')->find($arMovimientoDetalle['cuentaDevolucionVentas']);
                ...
                $arMovimientosDetalles = $em->getRepository('BrasaInventarioBundle:InvMovimientosDetalles')->DevResumenCuentaInventario($codigoMovimiento);

                foreach($arMovimientosDetalles as $arMovimientoDetalle) {
                    $arRegistro = $this->CrearEncabezadoRegistro($arMovimiento);
                    //Para las ventas
                    if($arMovimiento->getDocumentoRel()->getCodigoComprobanteContableFk() == 3)
                        $arCuentaContable = $em->getRepository('BrasaContabilidadBundle:CtbCuentasContables')->find($arMovimientoDetalle['cuentaInventario']);                    
                    $arRegistro->setCuentaRel($arCuentaContable);
    ...
    private function CrearEncabezadoRegistro ($arMovimiento) {
        $arRegistro = new \Brasa\ContabilidadBundle\Entity\CtbRegistros();                    
    ...
    public function ContabilizarMovimientoContabilidad($codigoMovimiento) {
        $em = $this->getEntityManager();
        $arMovimiento = new \Brasa\ContabilidadBundle\Entity\CtbMovimientos();
        $arMovimiento = $em->getRepository('BrasaContabilidadBundle:CtbMovimientos')->find($codigoMovimiento);
        if($arMovimiento->getEstadoContabilizado() == 0) {
            $arMovimientosDetalles = new \Brasa\ContabilidadBundle\Entity\CtbMovimientosDetalles();
            $arMovimientosDetalles = $em->getRepository('BrasaContabilidadBundle:CtbMovimientosDetalles')->findBy(array('codigoMovimientoFk' => $codigoMovimiento));
